...
class PayForAltService(
    ...
    fun payForAltAs(count: Int = 1, userId: Int, ip: String? = null): List<AltCategory> {
        ...
        if (correctCount > 0) {
            userCoin.balance -= (correctCount * 2)
            userCoinService.updateById(userCoin)
            coinTransactionHistoryService.logTransaction(userId = userId, amount = -(correctCount * 2), type = EnumTransactionType.PAID_USER_API_FETCH)
